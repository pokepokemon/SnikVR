...
namespace DigitalOpus.MB.MBEditor
{
    public class MB3_TextureBakerEditorInternal
    {
        ...
        public static string ValidatePlayerSettingsDefineSymbols()
        {
            ...
            MBVersionConcrete.PipelineType pipelineType = MBVersionConcrete.DetectPipeline();
            BuildTargetGroup targetGroup = EditorUserBuildSettings.selectedBuildTargetGroup;
            string scriptDefines = PlayerSettings.GetScriptingDefineSymbolsForGroup(targetGroup);

            string s = "";
            if (pipelineType == MBVersionConcrete.PipelineType.HDRP)
            {
                if (!scriptDefines.Contains(MBVersionConcrete.MB_USING_HDRP))
                {
